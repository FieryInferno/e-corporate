...
<body>
    ...
    <div class="w-100">
        <table class="table table-sm table-border-bottom">
            ...
            <tbody>
                <?php if ($getdata): ?>
                    <?php $totalpengajuan = 0 ?>
                    <?php $no=1; foreach ($getdata as $row): ?>
                        <?php $totalpengajuan = $totalpengajuan + $row['nominal'] ?>
                        <tr>
                            <td><?php echo $no++; ?></td>
                    ...
                    <tr class="bg-grey-300">
                        <td colspan="5">Total Pengajuan Kas Kecil 
                            <?php if ($tipe_cetak == '0') { ?>   
                                    Periode <?php echo formatdatemonthname($_GET['tanggalawal']) ?> - <?php echo formatdatemonthname($_GET['tanggalakhir']); } ?> </td>
                        <td class="text-right font-weight-bold"><?php echo "Rp. " .number_format($totalpengajuan, 2, ",", ".") ?></td>
                    </tr>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="text-center"><?php echo lang('data_not_found') ?></td>
                        </tr>
                <?php endif ?>
            </tbody>
        </table>
