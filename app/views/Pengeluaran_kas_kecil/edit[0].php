        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for(i=0; i<data.length; i++){
                    var jumlahkasitem = parseInt(data[i].jumkas) - parseInt(data[i].total);
                    table_detail.row.add([
                        data[i].itemid,
                        data[i].nama_item,
                        formatRupiah(String(data[i].harga), 'Rp. '),
                        data[i].jumlah,
                        formatRupiah(String(data[i].subtotal), 'Rp. '),
                        data[i].diskon,
                        data[i].ppn,
                        data[i].akunno,
                        formatRupiah(String(data[i].total), 'Rp. '),
   ...
   //nomor kwitansi
    $('#perusahaan').change(function(){ 
        ...
        $.ajax({
            ...
            dataType : 'json',
            success: function(data){
                var nomor_kwitansi = '{nokwitansi}';
                var kodeper = '';
                var i;
                for(i=0; i<data.length; i++){ kodeper += data[i].kode; }
                
                var convert = nomor_kwitansi.split("/"); 
